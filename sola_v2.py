...
'''
  File name: sola_v2.py
  Author: tofubridge
  Description: sola (GUI) reads daemon data in from an Otogi json file and writes
  the data to a csv file for easier data manipulation
...
def json_to_csv(savename):
    # load all data in from json file, making sure encoding is correct 
    with open(json_loaded, "r", encoding="utf-8") as reader:
    ...
    with open(savename + ".csv", "w", encoding="utf-8-sig") as csvfile:
...
# open_json asks the user for a json file to open
def open_json():
    global json_loaded
    json_loaded = filedialog.askopenfilename(title="Select the Otogi JSON File", \
                                            filetypes=types)
    if json_loaded[-5:] == ".json":
        status.text = json_loaded.split("/")[-1] + " selected!"
        status['text'] = json_loaded.split("/")[-1] + " selected!"
    else:
        json_loaded = False

# save_csv() writes the data from the user selected json file to a csv file at a
#  user selected location
def save_csv():
    global json_loaded
    if json_loaded is False:
        messagebox.showerror(title="Oops!", message="There is no .json to read from!")
    else:
        savefilename = filedialog.asksaveasfilename()
        json_to_csv(savefilename)
        json_loaded = False
        messagebox.showinfo(title="Conversion Complete",\
                        message="sola has finished writing to '{0}'"\
                        .format(savefilename.split("/")[-1]+".csv"))         
        status.text = "No .json file selected..."
        status['text'] = "No .json file selected..."
...
file_button = ttk.Button(mainframe, text="Open .json from File", command=open_json)
file_button.grid(column=0, row=0, sticky=(S, N, E, W))

export_button = ttk.Button(mainframe, text="Save As .csv", command=save_csv)
export_button.grid(column=1, row=0, sticky=(S, N, E, W))
export_button.bind("<Enter>", lambda e: export_button.configure(text= "Enter name without .csv!"))
export_button.bind("<Leave>", lambda e: export_button.configure(text= "Save As .csv"))

status = ttk.Label(mainframe, text="No .json file selected...")
status.grid(column=1, row=1, sticky=(S, N, E, W))
